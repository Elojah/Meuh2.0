...
/*   Updated: 2015/02/23 13:10:07 by erobert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_open_infile(t_cmd *cmd, char *str, int two)
{
	if (cmd->fd[0] > 0)
		close(cmd->fd[0]);
	if (two)
...
static void	ft_open_outfile(t_cmd *cmd, char *str, int two)
{
	if (cmd->fd[1] > 0)
		close(cmd->fd[1]);
	if (two)
...
static void	ft_open_redirection(char c, t_cmd *cmd, char *str, int two)
{
	char	**tab;
...
static int	ft_getfd(char c, t_cmd *cmd, char *str)
{
	while (*str)
	{
		if (*str == c)
