...
FragTrap::FragTrap(std::string name) :
	...
	_armorDamageReduction(5),
	_name(name) {
	std::cout << _name << "\t--\tstart bootup sequence.FragTrap style" << std::endl;
	std::srand(std::clock());
}
...
void	FragTrap::rangedAttack(std::string const & target) {
	std::cout << _name << "\tattacks\t" << target << "\twith ranged attack.FragTrap style" << std::endl;
}

void	FragTrap::meleeAttack(std::string const & target) {
	std::cout << _name << "\tattacks\t" << target << "\twith melee attack.FragTrap style" << std::endl;
}
...
void	FragTrap::takeDamage(unsigned int amount) {
	if (amount > _armorDamageReduction) {
		if (amount - _armorDamageReduction > _hitPoints) {
			_hitPoints = 0;
			std::cout << _name << "\ttook\t" << amount - _armorDamageReduction << "\tdamages.FragTrap style" << std::endl;
		...
		std::cout << _name << "\tarmor absorbed damages.FragTrap style" << std::endl;
	}
...
void	FragTrap::beRepaired(unsigned int amount) {
	...
	std::cout << _name << "\thas been repaired for\t" << amount << "\thp.FragTrap style" << std::endl;
	std::cout << "\t\t" << _hitPoints << "\tleft." << std::endl;
}
