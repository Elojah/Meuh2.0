...
void			GUIGL::initBuffers(void) {
	...
	for (size_t i = 0; i < _width; ++i) {
		for (size_t j = 0; j < _height; ++j) {
			...
			std::cout << "VBx:\t" << _index_buffer_data[n - 1] << std::endl;
			_index_buffer_data[n++] = (i * _height) + j + 1;
			std::cout << "VBy:\t\t" << _index_buffer_data[n - 1] << std::endl;
			_index_buffer_data[n++] = (i * _height) + j;
			std::cout << "VBy:\t\t\t" << _index_buffer_data[n - 1] << std::endl;
	...
	_progID = LoadShaders("./f3/src/shaders/Grid.vert", "./f3/src/shaders/Grid.frag");
