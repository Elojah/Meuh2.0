...
let k_nn (set: 'a list) (value:'a) : string =
	let rec loop_neighbours set n_good n_bad = match set with
		| (a, b)::next when b = "g" -> loop_neighbours next (n_good + 1) n_bad
		| (a, b)::next when b = "b" -> loop_neighbours next n_good (n_bad + 1)
		| (a, b)::next -> loop_neighbours next n_good n_bad
		| [] -> if n_good > n_bad then "That's a good value :)" else "That's a bad value :("
	in
	let neighbours = get_n_neighbours set value 3 in
	loop_neighbours neighbours 0 0
...
let () =
	if (Array.length (Sys.argv) <> 2) then
		...
		try
			...
			print_endline ( k_nn init_set ([|6.5; 7.5|], "I am the value") )
		with
